...
uint16_t  do_crc(unsigned char *q, int len)
{
    ...
    while (len-- > 0)
      crc = CRC16Table[((crc&0xff) ^ *q++) & 0xff] ^ (crc >> 8);
...
int main()
{
    uint8_t str[25]={0x84 ,0x3B ,0x00 ,0x00 ,0x02 ,0x31 ,0x83 ,0x80 ,0x09 ,0x2B ,0x0B ,0x13 ,0x03 ,0x02 ,0x14 ,0x21 ,0x00 ,0x00};
    uint16_t c=CRC16(str,18);
    printf("[%d]%x\n",18,c);
    c=do_crc(str,18);
    printf("[%d]%x\n",18,c);
    c=CRC16_IBM(str,18);
    printf("[%d]%x\n",18,c);
    c=CRC16_XMODEM(str,18);
    printf("[%d]%x\n",18,c);
    return 0;
}
#endif
